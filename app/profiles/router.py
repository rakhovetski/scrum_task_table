...
from app.profiles.schemas import SProfileLoginInfo, SProfileRegisterInfo
...
async def get_my_profile(my_profile: Profile = Depends(get_current_profile)):
    return my_profile


@auth_router.post('/register')
async def register_profile(profile_info: SProfileRegisterInfo):
...
async def login_profile(response: Response,
                        profile_info: SProfileLoginInfo):
